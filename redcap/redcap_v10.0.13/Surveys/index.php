...
if (trim($response_limit_custom_text) == "") {
	...
	$response_limit_custom_text = RCView::escape($response_limit_custom_text);
}
...
if (isset($_POST['submit-action']))
{
	...
	if (!isset($_GET['__reqmsg']))
	{
		...
		list ($fetched, $context_msg, $log_event_id, $dataValuesModified, $dataValuesModifiedIncludingCalcs) = DataEntry::saveRecord($fetched, true, false, false, $_POST['__response_id__'], true);
		// Set hidden edit
		$hidden_edit = 1;
	}
    ...
    || ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['__endpublicsurvey']) && isset($_GET['__rh'])))
{
	...
	else
	{
		...
		Survey::exitSurvey($full_acknowledgement_text, false);
	}
}
...
{
	?><script type="text/javascript">$(function(){ checkSecondaryUniqueField( $(':input[name="<?=$secondary_pk?>"]') ); });</script><?php
}
	...
	<table cellspacing="0" style="width:100%;max-width:100%;">
		<tr>
			...
			if ($text_to_speech > 0) {
				...
				// Buttons
				$text_to_speech_button = RCView::div(array('id'=>'text_to_speech_button', 'aria-hidden'=>"true", 'style'=>'margin-top:7px;text-align:center;position:relative;left:15px;'),
											// Enable button
