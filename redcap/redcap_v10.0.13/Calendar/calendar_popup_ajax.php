...
if (isset($_REQUEST['action'])) {
	switch ($_REQUEST['action']) {
		//Edit the visit date
		case "edit_date":
			$sql = "update redcap_events_calendar set event_date = '".db_escape($event_date)."' where cal_id = '".db_escape($_GET['cal_id'])."'";
			$q = db_query($sql);
			if (!$q) {
		...
		case "edit_time":
			...
			$sql = "update redcap_events_calendar set event_time = '".db_escape($_GET['event_time'])."' where cal_id = {$_GET['cal_id']}";
			$q = db_query($sql);
			if (!$q) {
				exit("<b>{$lang['global_01']}!</b>");
			} else {
		...
		case "edit_status":
			$sql = "update redcap_events_calendar set event_status = '".db_escape($_GET['event_status'])."' where cal_id = {$_GET['cal_id']}";
			$q = db_query($sql);
			if (!$q) {
				exit("<b>{$lang['global_01']}!</b>");
			} else {
		...
		//Edit notes
		case "edit_notes":
			$sql = "update redcap_events_calendar set notes = '".db_escape($_POST['notes'])."' where cal_id = {$_POST['cal_id']}";
			if (db_query($sql)) {
				//LOGGING
