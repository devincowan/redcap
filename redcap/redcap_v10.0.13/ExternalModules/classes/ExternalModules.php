...
//	static function enable($moduleDirectoryPrefix, $version)
	...
	public static function getProjectStatus($pid) {
		...
		if ($row !== null) {
			switch ($row["status"]) {
				...
				case 2: $status = empty($row["completed_time"]) ? "AC" : "DONE"; break;
			}
		}
		return $status;
	}

	function getRecordIdField($pid){
		$result = ExternalModules::query("
			select field_name
			from redcap_metadata
			where project_id = ?
	...
	function requireProjectId($pid = null)
	{
	...
	function detectParameter($parameterName, $value = null)
	{
		if($value == null){
	...
	function requireParameter($parameterName, $value)
	{
