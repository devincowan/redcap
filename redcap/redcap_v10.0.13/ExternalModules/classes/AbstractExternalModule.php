...
class AbstractExternalModule
{
	...
	public function addAutoNumberedRecord($pid = null){
		...
		if(!empty($results['warnings'])){
			error_log("Warnings occurred while calling " . __METHOD__ . "().  These should likely be ignored.  In fact, this error message could potentially be removed:" . json_encode($results, JSON_PRETTY_PRINT));
		}
	...
	public function __call($name, $arguments){
		...
		return ExternalModules::getFrameworkInstance($this->PREFIX)->callFromModuleInstance($name, $arguments);
	}

	// Allow framework object references like `records` to be returned directly.
	public function __get($name){
		return ExternalModules::getFrameworkInstance($this->PREFIX)->{$name};
	}
