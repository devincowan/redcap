...
class ExternalModulesTest extends BaseTest
{
	...
	function testGetLinks()
	{
		...
		$assertUrl = function($pageExpected, $actual){
			$expected = ExternalModules::getPageUrl(TEST_MODULE_PREFIX, $pageExpected);
	...
	function testInitializeFramework_unsupportedVersions(){
		...
		// FrameworkBaseTest tests actual use cases well.
	...
	function testEnsureAllFrameworkVersionsTested(){
		...
		while($version > 1){
			...
			if(!file_exists(__DIR__ . "/$filename")){
				throw new Exception("A $filename file must be created to ensure that unit tests run for that framework version.");
			}
