...
class AbstractExternalModuleTest extends BaseTest
{
	...
	function testSetSetting_projectDesignRights()
	{
		...
		$addToProject = function($pid, $design) use ($username){
			\UserRights::addPrivileges($pid, [
				'username' => $username,
				'forms' => [],
				'design' => $design,
			]);
	...
	private function getUsernameNotOnProject($pid){
		...
		// Make the username lowercase because some usernames are stored with a capitalized first character,
		// even though REDCap functions like UserRights::getPrivileges() expect them to be all lowercase.
		// This fixed the testSetSetting_projectDesignRights() unit test on username 'Crenshd'.
