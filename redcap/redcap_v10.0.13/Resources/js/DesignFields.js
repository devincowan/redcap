...
function checkMatrixFieldName(ob)
{
	...
	if (ob.val().length > 26) {
		simpleDialog(langOD7,null,'variable-name-too-long-warning');
	}
}
...
function checkFieldName(temp,submitForm)
{
	...
	var performSubmitWithChecks = function(){
		//Detect if longer than 26 characters. If so, give warning but allow.
		if (temp.length > 26 && !$('#field_name').prop('readonly')) {
			simpleDialog(langOD7,null,'variable-name-too-long-warning');
		}
...
function openLogicBuilder(field_name) {
	...
	$.post(app_path_webroot+'Design/branching_logic_builder.php?pid='+pid, { field_name: $('#logic_builder').attr('field'), action: 'get-logic' }, function(data){
		if (data != '0') {
			var json_data = jQuery.parseJSON(data);
			if (json_data.length < 1) {
				showProgress(0,1);
				...
				buttons: [{
					...
					click: function() { 
						...
						if(json_data.logic != branching_logic && json_data.same_logic_field == true && json_data.branching_status == "0") {
							$('#branching_update').dialog({ bgiframe: true, modal: true, width: 750, open: function(){fitDialog(this)},
								buttons: [{
									text: design_99,
	...
	//Pre-fill form if editing question
		...
		$.get(app_path_webroot+"Design/edit_field_prefill.php", { pid: pid, field_name: sq_id },
			function(data) {
				...
				if (ques['misc'] != '') {
					$('#field_annotation').css('height','60px');
				}
...
function toggleEmbeddedFieldsButtonDesigner(selector)
{
	...
	$('.rc-field-embed-designer-btn').removeClass('hide');
	$(selector).each(function(){
		$('.rc-field-embed-designer', this).removeClass('hide');
		$('.rc-field-embed-designer-btn', this).addClass('hide');
	});
}
...
function resetAddQuesForm() {
	...
	$('#field_annotation').css('height','40px');
	if (document.getElementById('question_num') != null) document.getElementById('question_num').value = '';
	if (document.getElementById('element_enum') != null) document.getElementById('element_enum').value = '';
...
function previewInstrument(view_as_form) {
	if (view_as_form) {
		...
		$('.frmedit_icons .rc-field-embed-designer:not(.hide)').each(function(){
...
function checkEnumRawVal(ob) {
	...
	if (dupCodeWarning != '') {
		dupCodeWarning = "<div class='red'>"+langOD54+"<br><b>"+dupCodeWarning+"</b></div>";
	}
	...
	if (choices_fixed.length > 0) {
		...
		simpleDialog(dupCodeWarning,langOD30);
	}
	return true;
}
...
function chkVarFldDisabled() {
	if (status > 0 && $('#field_name').attr('readonly')) {
		$('#field_label').focus();
		simpleDialog(langOD47,null,'varnameprod-nochange',null,"$('#field_label').focus();",null);
	}
}
