...
function enableRuleTableEdit() {
	...
	$('#table-rules .editlogic').click(function(){
		...
		var thisRuleId = $(this).attr('rid');
		$(this).html( '<textarea id="input_rulelogic_id_'+thisRuleId+'" class="x-form-field notesbox" style="font-size:11px;height:60px;margin:4px 0;width:95%;" onblur=\'if(!checkLogicErrors(this.value,1)){validate_logic(this.value,"",0);}\'>'+thisRuleLogic+'</textarea>'
...
function executeRulesAjax(rule_ids,show_exclusions,replaceRuleTable,action) {
	...
	// Ajax request
	var thisAjax = $.post(app_path_webroot+'DataQuality/execute_ajax.php?pid='+pid, { record: $('#dqRuleRecord').val(), rule_ids: rule_ids, show_exclusions: show_exclusions, action: action }, function(data){
		// Get data returned
		if (data == '') {
		...
		if (replaceRuleTable == 0) {
			...
			var discrep_text = "<div style='float:left;font-size:15px;width:50px;text-align:center;"+textColor+"'>"+discrepf+"</div>"
							 + "<div style='float:right;'><a href=\"javascript:;\" onclick=\"viewResults('"+rule_id+"','"+escapeHtml(title)+"');\" style='font-size:11px;text-decoration:underline;'>view</a></div>"
