...
class RedCapDB {
	...
	function saveAPIRights($username, $project_id, $export, $import, $mobile_app) {
		$sql = array();
		try {
			$sql[] = $this->save('redcap_user_rights', array('api_export' => $export, 'api_import' => $import, 'mobile_app' => $mobile_app),
	 ...
	 */
	function getAPIToken($username, $project_id) {
		$sql = "SELECT api_token FROM redcap_user_rights WHERE username = ? AND project_id = ?";
		$objs = $this->getObjects($sql, array($username, $project_id));
		if (count($objs) === 0) return null;
		return empty($objs[0]->api_token) ? null : $objs[0]->api_token;
	}
	...
	function getAPITokens($orderProj=false, $project_id='') {
		...
		$sql = "SELECT u.api_token, lower(u.username) as username, p.project_id, p.app_title, u.api_export, u.api_import, u.mobile_app,
			r.api_export as api_export_role, r.api_import as api_import_role, r.mobile_app as mobile_app_role
		...
		foreach ($rights as $key=>&$ob) {
			...
			unset($ob->api_export_role, $ob->api_import_role, $ob->mobile_app_role);
		}
		return $rights;
	}

	/** Returns a count of the number of API tokens assigned on the given project. */
	function countAPITokensByProject($project_id) {
		$sql = "SELECT COUNT(*) AS TokenCount
			FROM redcap_user_rights
			WHERE project_id = ? AND api_token IS NOT NULL";
		$objs = $this->getObjects($sql, array($project_id));
		return $objs[0]->TokenCount;
	}

	function setAPITokenSuper($username)
	{
		$username = db_escape($username);
		$tok = strtoupper(hash('sha256', "$username&" . microtime()));
	...
	function setAPIToken($username, $project_id) {
		$sql = array();
		try {
			$tok =  strtoupper(md5("$username&$project_id&" . microtime()));
	...
	function getLastAPICallDateSuper($username)
	{
		...
		{
			$row = db_fetch_assoc($q);
			return DateTimeRC::format_ts_from_ymd(DateTimeRC::format_ts_from_int_to_ymd($row['ts']));
		}
	...
	function getLastAPICallDates($username=null, $project_id=null) {
		...
		foreach ($objs as $obj) $calls[$obj->user][$obj->project_id] = $obj;
		return $calls;
	}
	...
	function getProjects() {
		$sql = "SELECT * FROM redcap_projects ORDER BY app_title";
		...
		try {
			if (empty($ui_id)) { // insert
				...
				$sql[] = $this->save('redcap_user_information', array('username' => $username, 'user_email' => $email, 'user_email2' => $email2,
						...
						'api_token_auto_request' => $api_token_auto_request, 'fhir_data_mart_create_project' => $fhir_data_mart_create_project));
			}
			else { // update
