...
class PDF
{
	...
	public static function output()
	{
		...
		// Call the PDF hook
		$hookReturn = Hooks::call('redcap_pdf', array($project_id, $metadata, $data, $_GET['page'], $_GET['id'], $_GET['event_id'], $_GET['instance']));
			...
			foreach (array_keys($event_array) as $event_id)
			{
				...
				foreach ($eventNormalized as $event_id=>$nattr1)
				{
					...
					if ($record != '')
					{
						...
						if (isset($surveys_enabled) && $surveys_enabled && !isset($_GET['hideSurveyTimestamp']))
						{
					...
					foreach ($nattr1 as $repeat_instrument=>$nattr2)
					{
						foreach ($nattr2 as $repeat_instance=>$dataNormalized)
						{
								...
								// foreach ($this_metadata as $row)
							{
								...
								if ($dataNormalized[$row['field_name']] === DEID_TEXT) {
									$row['element_type'] = "text";
								}
									...
									if ($row['custom_alignment'] == 'LV' || $row['custom_alignment'] == 'LH')
									{
										...
										if (isset($dataNormalized[$row['field_name']]) && $row['element_type'] == "file" && $row['element_validation_type'] == "signature") {
											...
											if (!USE_UTF8) {
												...
												$pdf = PDF::new_page_check(6, $pdf, $y_units_per_line, $bottom_of_page, $study_id_event);
												// Get current position, so we can reset it back later
												$y = $pdf->GetY();
												$x = $pdf->GetX();
												// Copy file to temp directory
									...
									else
									{
										...
										if (isset($dataNormalized[$row['field_name']]) && $row['element_type'] == "file" && $row['element_validation_type'] == "signature") {
											...
											if (!USE_UTF8) {
												...
												// Check for new page
												$counter = (count($q_lines) >= 6) ? count($q_lines) : 6;
												$pdf = PDF::new_page_check($counter, $pdf, $y_units_per_line, $bottom_of_page, $study_id_event);
												// If a survey and using custom question numbering, then render question number
												$pdf = PDF::addQuestionNumber($pdf, $row_height, $row['question_num'], (isset($isSurvey) && $isSurvey), $customQuesNum, $num);
												// Get current position, so we can reset it back later
												...
												if ($sigfile_path !== false) {
													...
													unlink($sigfile_path);
													// Set new position
													if ($y+round($sigfile_size[1]/6) > $y2) $y2 = $y+round($sigfile_size[1]/6);
													$pdf->SetY($y2);
												}
											}
											// Add field note, if applicable
	...
	public static function renderMatrixHeaderRow($pdf,$hdrs=array(),$page_width,$matrix_label_width)
	{
	...
	public static function renderMatrixRow($pdf,$label,$hdrs=array(),$enumData,$row_height,$sigil_width,$page_width,$matrix_label_width,$bottom_of_page,$study_id_event,$isCheckbox)
	{
