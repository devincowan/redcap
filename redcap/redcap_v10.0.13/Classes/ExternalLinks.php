...
class ExternalLinks
{
	...
	{
		global $lang, $longitudinal, $Proj, $user_rights, $isAjax, $isIE;
		...
		if (db_num_rows($q) > 0)
		{
			...
			while ($row = db_fetch_assoc($q))
			{
				...
				if (is_numeric($row['project_id'])) {
					$extLinkImg = "<i class='fas fa-sign-out-alt ml-1'></i>";
				} else {
				...
				if ($row['link_type'] == 'POST_AUTHKEY')
				{
					...
					$this_link =   "<form $openNewWin id='$extLinkFormName' action='{$row['link_url']}' method='post' enctype='multipart/form-data'>
										...
										$extLinkImg <a href='javascript:;' onclick=\"ExtLinkClickThru({$row['ext_id']},{$row['open_new_window']},'".js_escape($row['link_url'])."','$extLinkFormName'); $submitForm return true;\">{$row['link_label']}</a>
									</form>";
				}
				...
				else
				{
					...
					// Link HTML
					$this_link = "$extLinkImg <a href=\"$href\" $openNewWin onclick=\"ExtLinkClickThru({$row['ext_id']},{$row['open_new_window']},'".js_escape($row['link_url'])."','');return true;\">{$row['link_label']}</a>";
				}
				// Add link to menu
				if (is_numeric($row['project_id'])) {
		...
		{
			$output .= '<div id="global_ext_links" class="menubox" style="border-bottom:1px solid #D0D0D0;padding-left:9px;padding-top:0;margin-top:0;">
