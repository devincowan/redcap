...
if (isset($_POST['question']) && isset($_POST['answer']) && is_numeric($_POST['question']) && isset($questions[$_POST['question']]) && isset($_POST['user_email']) && isEmail($_POST['user_email']))
{
	...
	$answerHash = Authentication::hashSecurityAnswer($_POST['answer'], USERID);
	// Set email address (if different)
	if ($user_email != $_POST['user_email'])
	{
		$sql = $sql_all[] = "update redcap_user_information set user_email = '".db_escape($_POST['user_email'])."' where username = '".db_escape(USERID)."'";
		db_query($sql);
	}
	// Add to table
	$sql = $sql_all[] = "update redcap_auth set password_question = {$_POST['question']}, password_answer = '".db_escape($answerHash)."',
			password_question_reminder = null where username = '".db_escape(USERID)."'";
	if (db_query($sql))
	{
		// Logging
		Logging::logEvent(implode(";\n", $sql_all),"redcap_auth","MANAGE",USERID,"username = '" . db_escape(USERID) . "'","Set up user security question");
...
elseif (isset($_POST['setreminder']))
{
	...
	// Update table
	$sql = "update redcap_auth set password_question_reminder = '$in2days' where username = '".db_escape(USERID)."'
