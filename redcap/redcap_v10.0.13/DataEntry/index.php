...
if ($auto_inc_set)
{
	...
	{
		if (Records::recordExists(PROJECT_ID, $fetched)) {
			// Record already exists, so generate the next one
			$fetched = DataEntry::getAutoId();
			if (isset($_POST['submit-action'])) {
...
if (isset($_POST['submit-action']))
{
	...
	switch ($_POST['submit-action'])
	{
		...
		case 'save-and-redirect':
			...
			list ($fetched, $context_msg, $log_event_id, $dataValuesModified, $dataValuesModifiedIncludingCalcs) = DataEntry::saveRecord($fetched, true, false, false, null, true);
...
elseif (isset($_GET['id']))
{
	...
	if ($user_rights['forms'][$_GET['page']] == '2') {
		...
		if ($isRepeatingForm) {			
			...
			$next_instance_button_text = (max(array_keys($instances)) == $_GET['instance'] || !isset($instances[$_GET['instance']])) ? $lang['data_entry_275'] : $lang['data_entry_276'];
	...
	*/
	...
	// REPEATING FORMS TABS
	print $repeatingFormTabs;
